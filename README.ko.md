<div align="center">
  ...
  <p><strong>Codex OAuth Responses를 통해 Python에서 이미지를 생성합니다</strong></p>
...
`codex-image-gen`은 Codex OAuth와 Responses API `image_generation` 도구를
통해 고품질 이미지를 생성하는 작고 의존성 없는 Python 라이브러리입니다.

기존 Codex 로그인 인증(`~/.codex/auth.json`)을 그대로 사용하며, Codex OAuth
bridge에 raw Responses payload를 전달한 뒤 디코딩된 이미지 bytes와
메타데이터를 Python dataclass로 반환합니다. `OPENAI_API_KEY`는 사용하지
않습니다.
...
| | |
| --- | --- |
| **고품질 이미지**<br>Codex OAuth Responses를 통해 `gpt-image-2`를 사용합니다. | **다양한 입력 타입**<br>로컬 경로, URL, data URL, file ID, `detail` 매핑을 사용할 수 있습니다. |
| **안정적인 예외 처리**<br>OAuth bridge 실패, 잘못된 JSON, 누락된 이미지 결과, 잘못된 base64를 명확한 예외로 감쌉니다. | **집중된 파라미터 표면**<br>Codex bridge가 사용하는 gpt-image-2 옵션을 전달합니다. |
| **타입 있는 결과**<br>이미지 bytes, MIME type, response ID, call ID, revised prompt, partial image를 frozen dataclass로 반환합니다. | **무거운 의존성 없음**<br>런타임은 Python 표준 라이브러리만 사용합니다. |
...
- size, output format, output compression, background, moderation, partial
  images, reasoning 설정, text verbosity, timeout, custom instructions를 일급
  옵션으로 제공합니다.
...
partial image는 streaming Responses 요청이 완료된 뒤 결과 객체에 수집됩니다.
이 함수는 실시간 streaming callback을 제공하지 않습니다.
...
이 라이브러리는 Codex Responses bridge가 사용하는 파라미터만 노출합니다.

- 기본 mainline Responses model은 `gpt-5.5`입니다.
- 이미지 생성 도구에는 항상 `model="gpt-image-2"`를 보냅니다.
- `input_image_mask`는 Responses `image_generation` tool에 문서화되어 있어
  노출합니다. 이 라이브러리는 mask를 그대로 전달하며, 필요한 alpha channel을
  자동 생성하지는 않습니다.
...
- `previous_response_id`와 이전 `image_generation_call` item 참조는 노출하지
  않습니다. 후속 편집에는 이미지 bytes, 로컬 파일, URL, file ID를 사용하세요.
